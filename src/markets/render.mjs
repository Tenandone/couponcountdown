...
function metrics(q,l){const t=weeklyCopy[l];return `<dl class="weekly-metrics">${[['price',t.current],['previousPrice',t.previous]].map(([key,label])=>`<div><dt>${label}</dt><dd>${value(q[key],l)}</dd></div>`).join('')}</dl>`;}
const stamp=(s,l)=>date(s,l)+' · '+new Date(s).toISOString().slice(11,16)+' UTC';
function times(q,l){const t=weeklyCopy[l];return q.collectedAt?`<p class="market-note">${t.updated}: ${stamp(q.collectedAt,l)}<br>${t.observed}: ${q.assetType==='fx'?date(q.observedAt,l):stamp(q.observedAt,l)}${q.previousCollectedAt?`<br>${t.previousTime}: ${stamp(q.previousCollectedAt,l)}`:''}</p>`:'';}
function comparison(q,l){const t=weeklyCopy[l];return !q.price?'':q.previousPrice==null?`<p class="market-status">${t.baseline}<br>${t.nextBaseline}</p>`:`<p class="market-note">${t.change}: <strong>${delta(q.changePercent,l)}</strong></p>`;}
...
export function marketPage(l,report,origin,resources,ga,{slug='',archive=false,archiveIds=[]}={}){
 ...
 const period=`<p class="weekly-period">${t.period}<br><strong>${date(report.collectedAt,l)}</strong></p>`;
 ...
 if(q){content+=`<section class="market-quote"><p class="market-status">${q.status==='stale'?t.stale:status(q,t)}</p>${metrics(q,l)}${comparison(q,l)}<p class="market-note">${unit(q,base)}</p>${times(q,l)}</section><section class="market-method"><h2>${base.method}</h2><p>${q.assetType==='fx'?t.fxMethod:t.method}</p>${q.source?`<p>${base.source}: <a href="${escape(q.source.url)}">${escape(q.source.name)}</a></p>`:''}</section>`;}
 else{
 ...
 content+=`<div class="market-list">${ready.map(a=>`<article class="weekly-asset"><a class="market-row" href="/${l}/markets/${a.slug}/" data-market-event="market_asset_click" data-asset="${a.slug}" data-source-section="weekly_asset_list"><span><strong>${escape(nameFor(a.slug,l))}</strong><small>${escape(a.symbol)}</small></span><span class="weekly-change ${a.changePercent<0?'down':'up'}">${a.previousPrice==null?'':delta(a.changePercent,l)} <span aria-hidden="true">→</span></span></a>${a.status==='stale'?`<p class="market-status">${t.stale}</p>`:''}${metrics(a,l)}${comparison(a,l)}<small class="market-note">${unit(a,base)} · <a href="${escape(a.source.url)}">${escape(a.source.name)}</a>${a.assetType==='fx'?' · '+t.fxMethod:''}</small>${times(a,l)}</article>`).join('')}</div>`;
